...
export function assetRoutes(app: FastifyInstance): void {
  ...
  app.post<{ Body: { url?: unknown; name?: unknown } }>('/assets/fetch', async (request, reply) => {
    ...
    if (!response.ok || !response.body) {
      return reply.status(502).send({
        ...
        error: {
          code: 'FETCH_FAILED',
          message: `Origin responded with ${String(response.status)}`,
        },
      });
    }
